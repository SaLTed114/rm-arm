...
int arm_config_validate(const arm_config_t *config) {
  ...
  if (!arm_dof_is_valid(config->dof)) return ARM_ERR_DOF;

  for (uint8_t i = 0u; i < config->dof; ++i) {
    const arm_joint_config_t *joint = &config->joints[i];
    if (joint->sign == ARM_REAL_ZERO) return ARM_ERR_CONFIG;
    if (joint->torque_limit_nm < ARM_REAL_ZERO) return ARM_ERR_CONFIG;
  }
...
int arm_init(arm_t *arm, const arm_config_t *config) {
  if (!arm || !config) return ARM_ERR_NULL;
