...
static bool torque_pushes_deeper_into_limit(
  ...
  if (q_rad <= joint->q_min_rad + margin && tau_nm < ARM_REAL_ZERO) {
    return true;
  }
  if (q_rad >= joint->q_max_rad - margin && tau_nm > ARM_REAL_ZERO) {
    return true;
  }
  return false;
}
...
static bool torque_pushes_with_overspeed(
    ...
    arm_real_t dq_rad_s,
    arm_real_t tau_nm) {
  if (joint->dq_limit_rad_s <= ARM_REAL_ZERO) {
    return false;
  }
  if (dq_rad_s >= joint->dq_limit_rad_s && tau_nm > ARM_REAL_ZERO) {
    return true;
  }
  if (dq_rad_s <= -joint->dq_limit_rad_s && tau_nm < ARM_REAL_ZERO) {
    return true;
...
int arm_safety_apply(const arm_safety_t *safety, const arm_state_t *state, arm_command_t *command) {
  if (!safety || !state || !command) {
    return ARM_ERR_NULL;
  }
  if (safety->dof == 0u || safety->dof > ARM_DOF_MAX || state->dof != safety->dof ||
      command->dof != safety->dof) {
